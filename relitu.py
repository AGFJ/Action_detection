...
def parse_args():
    ...
    parser.add_argument('-v', '--version', default='yowo_nano', type=str, choices=['yowo', 'yowo_nano'],
                        help='build YOWO')
    parser.add_argument('--weight', default=r'weights/ucf24/yowo_nano/yowo_nano_epoch_5.pth',
                        type=str, help='Trained state_dict file path to open')
...
def main():
    ...
    weights_dict = torch.load("weights/ucf24/weights/yowo_nano_epoch_5_80.3.pth", map_location='cpu')

    model.load_state_dict(weights_dict, strict=False)  # 加载自己模型的权重
    # target_layers = [model.backbone.layer4]
    target_layers = [model.channel_encoder]  # 定义目标层
